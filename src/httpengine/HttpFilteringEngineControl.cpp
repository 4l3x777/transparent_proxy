...
//#include <boost/predef/compiler.h>
	...
	#include "httpengine/mitm/secure/WindowsInMemoryCertificateStore.hpp"
#else
	...
	namespace httpengine
	{
			...
			m_onMessageEnd(onMessageEnd)
		{
			if (m_store == nullptr)
			{
				...
				{
					throw std::runtime_error("In HttpFilteringEngineControl::Start() - Failed to establish certificate trust with OS.");
				}
			}
		...
		void HttpFilteringEngineControl::Start()
		{
			...
			if (m_isRunning == false)
			{
				...
				{
					m_service.reset(new boost::asio::io_context());
				}
				else					
				{					
					m_service->reset();
				...
				for (uint32_t i = 0; i < m_proxyNumThreads; ++i)
				{
					m_proxyServiceThreads.emplace_back(
						std::thread
							{ 
								std::bind(
								static_cast<size_t(boost::asio::io_context::*)()>(&boost::asio::io_context::run), 
