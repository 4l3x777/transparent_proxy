...
#include "httpengine/mitm/diversion/DiversionControl.hpp"
		...
		namespace mitm
		{
			namespace diversion
			{
						...
						)
				{
					...
					#ifndef NDEBUG
						assert(m_diverter != nullptr && "In DiversionControl::DiversionControl(util::cb::MessageFunction, util::cb::MessageFunction, util::cb::MessageFunction) - Failed to allocate diverter class");
					#else
						if (m_diverter == nullptr)
						{
							throw std::runtime_error("In DiversionControl::DiversionControl(util::cb::MessageFunction, util::cb::MessageFunction, util::cb::MessageFunction) - Failed to allocate diverter class");
						}
					#endif
				}
