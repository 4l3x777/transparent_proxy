...
#include "httpengine/mitm/diversion/impl/win/WinDiverter.hpp"
		...
		namespace mitm
		{
			namespace diversion
			{
					...
					)
				{
					...
					{
						throw std::runtime_error("In WinDiverter::WinDiverter(...) - No valid firewall check callback was supplied.");
					}
				...
				void WinDiverter::Run()
				{
					...
					if (m_running == false)
					{
						...
						//static const char* FILTER_STRING = "outbound and tcp and ((ip and ip.SrcAddr != 127.0.0.1) or (ipv6 and ipv6.SrcAddr != ::1))";
						static const char* FILTER_STRING = "outbound and tcp";

						#ifdef HTTP_FE_BLOCK_TOR
							m_diversionHandle = WinDivertOpen("outbound and tcp", WINDIVERT_LAYER_NETWORK, -1000, 0);
						#else
							/*const char* errorStr;
							uint32_t errorPos;
							if (!WinDivertHelperCheckFilter(FILTER_STRING, WINDIVERT_LAYER_NETWORK, &errorStr, &errorPos))
							{
								...
								std::string errzzz("at pos");
								errzzz.append(std::to_string(errorPos));
								ReportError(errzzz);
							}*/

							m_diversionHandle = WinDivertOpen(FILTER_STRING, WINDIVERT_LAYER_NETWORK, -1000, 0);
						#endif

						m_quicBlockHandle = WinDivertOpen("udp and (udp.DstPort == 80 || udp.DstPort == 443)", WINDIVERT_LAYER_NETWORK, 0, WINDIVERT_FLAG_DROP);
						...
						WinDivertSetParam(m_diversionHandle, WINDIVERT_PARAM_QUEUE_LENGTH, 8192);
				...
				void WinDiverter::RunDiversion(LPVOID diversionHandlePtr)
				{
					...
					while (m_running)
					{
						...
						UINT pAddrLen = sizeof(addr);
						if (!WinDivertRecvEx(divertHandle, readBuffer.data(), PacketBufferLength, &recvLength, 0, &addr, &pAddrLen, &recvOverlapped))
						{
						...
						if (!WinDivertRecv(divertHandle, readBuffer.data(), PacketBufferLength, &recvLength, &addr))
						{
						...
						{	
							...
							// succeeded.
							WinDivertHelperParsePacket(readBuffer.data(), recvLength, &ipV4Header, &ipV6Header, nullptr, nullptr, nullptr, &tcpHeader, nullptr, &payloadBuffer, &payloadLength, nullptr, nullptr);
									...
									if (isLocalIpv4)
									{
										...
										if (payloadBuffer != nullptr)
										{
											...
											if (IsSocksProxyConnect(static_cast<uint8_t*>(payloadBuffer), payloadLength))
											{
												...
												ReportInfo("Blocking SOCKS proxy.");
						...
						if (modifiedPacket)
						{
							WinDivertHelperCalcChecksums(readBuffer.data(), recvLength, &addr, 0);
						}
						else
						{
							WinDivertHelperCalcChecksums(readBuffer.data(), recvLength, &addr, 0);
						...
						if (!WinDivertSendEx(divertHandle, readBuffer.data(), recvLength, 0, 0, &addr, sizeof(addr), nullptr))
						{
				...
				std::string WinDiverter::GetPacketProcessBinaryPath(const unsigned long processId) const
				{
					...
					if (processHandle != nullptr && processHandle != INVALID_HANDLE_VALUE)
					{
						...
						{
							std::string err("In WinDiverter::GetPacketProcessBinaryPath(const unsigned long) - Failed to get binary path using pid ");
							err.append(std::to_string(processId)).append(".");
							ReportError(err);
					...
					{
						#ifndef NDEBUG
						std::string err("In WinDiverter::GetPacketProcessBinaryPath(const unsigned long) - Failed to open process to query binary path using pid ");
						err.append(std::to_string(processId)).append(".");
						ReportError(err);
						#endif
					}
				...
				DWORD WinDiverter::GetPacketProcess(uint16_t localPort, uint32_t localV4Address, PMIB_TCPTABLE2* table, DWORD& currentTableSize)
				{
					...
					{
						currentTableSize = 0;
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t, PMIB_TCPTABLE2, DWORD&) - Failed to initialize table.");
						return 0;
					}
					...
					if ((dwRetVal = GetTcpTable2(*table, &currentTableSize, FALSE)) == ERROR_INSUFFICIENT_BUFFER)
					{
						...
						*table = static_cast<PMIB_TCPTABLE2>(malloc(currentTableSize));
						...
						{
							currentTableSize = 0;
							ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t, PMIB_TCPTABLE2, DWORD&) - Failed to resize table.");
							return 0;
						}
					...
					{
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t, PMIB_TCPTABLE2, DWORD&) - Failed to populate table.");
				...
				DWORD WinDiverter::GetPacketProcess(uint16_t localPort, uint32_t* localV6Address, PMIB_TCP6TABLE2* table, DWORD& currentTableSize)
				{
					...
					if (!table)
					{
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t[4], PMIB_TCP6TABLE2, DWORD&) - Expected pointer to TABLE structure, got nullptr!");
						return 0;
					}

					if (localV6Address == nullptr)
					{
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t[4], PMIB_TCP6TABLE2, DWORD&) - Expected uint32_t array with length of four for localV6Address, got nullptr!");
						return 0;
					}
					...
					{
						currentTableSize = 0;
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t[4], PMIB_TCP6TABLE2, DWORD&) - Failed to initialize table.");
						return 0;
					}
					...
					if ((dwRetVal = GetTcp6Table2(*table, &currentTableSize, FALSE)) == ERROR_INSUFFICIENT_BUFFER)
					{
						...
						*table = static_cast<PMIB_TCP6TABLE2>(malloc(currentTableSize));
						...
						{
							currentTableSize = 0;
							ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t[4], PMIB_TCP6TABLE2, DWORD&) - Failed to resize table.");
							return 0;
						}
					...
					{
						ReportError("In WinDiverter::GetPacketProcess(uint16_t, uint32_t[4], PMIB_TCP6TABLE2, DWORD&) - Failed to populate table.");
