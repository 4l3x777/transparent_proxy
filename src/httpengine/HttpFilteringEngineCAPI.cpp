...
#include <httpengine/HttpFilteringEngineCAPI.h>
	...
	)
{
	...
	#if BOOST_OS_WINDOWS
		#ifndef NDEBUG
			assert(firewallCb != nullptr && "On Windows, a valid firewall callback is required!");
		#endif
	...
	try
	{
		...
		#ifndef NDEBUG
			assert(inst != nullptr && "In fe_ctl_create(FirewallCheckCallback, ReportMessageCallback, \
...
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_start(PVOID) - Supplied HttpFilteringEngineCtl ptr is nullptr!");
	#endif
	...
	assert(success == true && "In fe_ctl_start(PVOID) - Caught exception and failed to start.");
...
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_stop(PVOID) - Supplied HttpFilteringEngineCtl ptr is nullptr!");
	#endif
	...
	assert(success == true && "In fe_ctl_stop(PVOID) - Caught exception and failed to stop.");
}

const bool fe_ctl_is_running(PVOID ptr)
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_is_running(PVOID) - Supplied HttpFilteringEngineCtl ptr is nullptr!");
	#endif
	...
	assert(success == true && "In fe_ctl_is_running(PVOID) - Caught exception and failed to check status.");
...
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_get_http_listener_port(PVOID) - Supplied HttpFilteringEngineCtl ptr is nullptr!");
	#endif
	...
	assert(success == true && "In fe_ctl_get_http_listener_port(PVOID) - Caught exception and failed to get HTTP listener port.");
...
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_get_https_listener_port(PVOID) - Supplied HttpFilteringEngineCtl ptr is nullptr!");
	#endif
	...
	assert(success == true && "In fe_ctl_get_https_listener_port(PVOID) - Caught exception and failed to get HTTPs listener port.");
...
{
	#ifndef NDEBUG
		assert(ptr != nullptr && "In fe_ctl_get_rootca_pem(char**, size_t*) - Supplied PVOID ptr is nullptr!");
		assert(bufferPP != nullptr && "In fe_ctl_get_rootca_pem(char**, size_t*) - Supplied buffer pointer-to-pointer is nullptr!");
		assert(bufferSize != nullptr && "In fe_ctl_get_rootca_pem(char**, size_t*) - Supplied buffer size pointer is nullptr!");
	#endif
	...
	assert(callSuccess == true && "In fe_ctl_get_rootca_pem(...) - Caught exception and failed to fetch root CA certificate.");
}
