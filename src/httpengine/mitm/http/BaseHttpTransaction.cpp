...
#include <httpengine/mitm/http/BaseHttpTransaction.hpp>
	...
	namespace httpengine
	{
		namespace mitm
		{
			namespace http
			{
				...
				const boost::string_ref BaseHttpTransaction::ContentTypeText = "text/";

				const boost::string_ref BaseHttpTransaction::ContentTypeHtml = "html";

				const boost::string_ref BaseHttpTransaction::ContentTypeJson = "json";

				const boost::string_ref BaseHttpTransaction::ContentTypeImage = "image/";

				const boost::string_ref BaseHttpTransaction::ContentTypeCss = "css";

				const boost::string_ref BaseHttpTransaction::ContentTypeJavascript = "javascript";
				...
				const bool BaseHttpTransaction::Parse(const size_t bytesReceived)
				{
					...
					{
						ReportError("In BaseHttpTransaction::Parse(const size_t&) - Upgrade requested. Unsupported.");
						return false;
					}

					if (m_httpParser->http_errno != 0)
					{
						std::string errMsg("In BaseHttpTransaction::Parse(const size_t&) - Failed to parse payload. Got http_parser error: ");
					...
					{
						ReportError("In BaseHttpTransaction::Parse(const size_t&) - Not all bytes were parsed. Unknown error occurred.");
						return false;
					}
				...
				const bool BaseHttpTransaction::IsPayloadChunked() const
				{
					...
					{
						if (boost::iequals(contentEncoding.first->second, "chunked"))
						{
							return true;
						}
					}
				...
				const bool BaseHttpTransaction::CompressGzip()
				{
					if (m_payload.size() == 0)
					{
						ReportWarning("In BaseBridge::CompressDeflate() - There is no payload to compress.");
						return false;
					}
				...
				const bool BaseHttpTransaction::DecompressPayload()
				{
					...
					std::string gzipEnc("gzip");
					std::string deflateEnc("deflate");
				...
				const bool BaseHttpTransaction::DecompressGzip()
				{
					if (m_payload.size() == 0)
					{
						ReportError("In BaseBridge::DecompressGzip() - There is no payload to decompress.");
						return false;
					}
					...
					{
						std::string errMessage("In BaseBridge::DecompressGzip() - Exception while decompressing: ");
						errMessage.append(e.what());
						ReportError(errMessage);
						return false;
					}					
				...
				const bool BaseHttpTransaction::DecompressDeflate()
				{
					if (m_payload.size() == 0)
					{
						ReportError("In BaseBridge::DecompressDeflate() - There is no payload to decompress.");
						return false;
					}
					...
					{
						std::string errMessage("In BaseBridge::DecompressDeflate() - Exception while decompressing: ");
						errMessage.append(e.what());
						ReportError(errMessage);
						return false;
					}
				...
				const bool BaseHttpTransaction::ConvertPayloadFromChunkedToFixedLength()
				{
					...
					if (!parser)
					{
						std::string error("In BaseHttpTransaction::ConvertPayloadFromChunkedToFixedLength() - Failed to allocate memory for http-parser.");
						ReportError(error);
						return false;
					}
					...
					{
						auto errorMessage = std::string("In BaseHttpTransaction::ConvertPayloadFromChunkedToFixedLength() - Got http_parser error: ");
					...
					{
						std::string errorMessage("In BaseHttpTransaction::ConvertPayloadFromChunkedToFixedLength() - Finalized payload is empty.");
					...
					else
					{
						...
						{
							std::string errorMessage("In BaseHttpTransaction::ConvertPayloadFromChunkedToFixedLength() - Failed to decompress payload.");
							ReportError(errorMessage);							
							free(parser);
							return false;
						}
				...
				int BaseHttpTransaction::OnMessageBegin(http_parser* parser)
				{
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnMessageBegin() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnMessageBegin() - http_parser is nullptr, somehow. \
				...
				int BaseHttpTransaction::OnHeadersComplete(http_parser* parser)
				{
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnHeadersComplete() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnHeadersComplete() - - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnMessageComplete(http_parser* parser)
				{
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnMessageComplete() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
						...
						{	
							...
							//TODO: change to decode transfered chunked paylaod
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnMessageComplete() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnChunkHeader(http_parser* parser)
				{
					...
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnChunkHeader() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnChunkHeader() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnChunkComplete(http_parser* parser)
				{
					...
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnChunkComplete() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnChunkComplete() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnHeaderField(http_parser* parser, const char *at, size_t length)
				{
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnHeaderField() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
						...
						if (length == 0)
						{
							trans->ReportError("In BaseHttpTransaction::OnHeaderField() - Length provided for the parsed header field/name is zero.");
							return -1;
						}
					}
					else
					{
						throw std::runtime_error("In BaseHttpTransaction::OnHeaderField() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnHeaderValue(http_parser* parser, const char *at, size_t length)
				{
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{	
							throw std::runtime_error("In BaseHttpTransaction::OnHeaderValue() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
						...
						{
							trans->ReportError(std::string("In BaseHttpTransaction::OnHeaderValue() - OnHeaderValue called while BaseHttpTransaction::m_lastHeader was empty."));
							return -1;							
						}
					}
					else 
					{
						throw std::runtime_error("In BaseHttpTransaction::OnHeaderValue() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
				...
				int BaseHttpTransaction::OnBody(http_parser* parser, const char *at, size_t length)
				{
					...
					if (parser != nullptr)
					{
						...
						if (trans == nullptr)
						{
							throw std::runtime_error("In BaseHttpTransaction::OnBody() - http_parser->data is nullptr when it should contain a pointer the http_parser's owning BaseHttpTransaction object.");
						}
					...
					{
						throw std::runtime_error("In BaseHttpTransaction::OnBody() - http_parser is nullptr, somehow. \
							This should never be allowed to happen ever because this apparently null object is supposed to be invoking \
							this callback, but if you're reading this, somehow it did. Welcome to the twilight zone. Nobody can wag \
							their finger at me for not wrapping a raw pointer with a null check.");
					}
