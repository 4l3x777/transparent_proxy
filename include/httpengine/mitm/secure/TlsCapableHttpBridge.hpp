...
#include "httpengine/network/SocketTypes.hpp"
#include "BaseInMemoryCertificateStore.hpp"
#include "httpengine/mitm/http/HttpRequest.hpp"
#include "httpengine/mitm/http/HttpResponse.hpp"
#include "httpengine/util/cb/EventReporter.hpp"
#include "httpengine/util/cb/StreamCopyUtils.hpp"
#include "util/http/KnownHttpHeaders.hpp"
#include "httpengine/util/hash/StringHashUtils.hpp"
		...
		namespace mitm
		{
			...
			{								
				...
				private:
					...
					/// </summary>
					boost::asio::io_context::strand m_upstreamStrand;

					/// <summary>
					/// For ensuring that asynchronous operation callback handlers involving the
					/// downstream client connection are not concurrently executed.
					/// </summary>
					boost::asio::io_context::strand m_downstreamStrand;
				...
				private:
					...
					class PreviewParser
					{
						...
						public:
							...
							const ParseResult Parse(const char* data, const size_t dataLength, std::string& outHost)
							{
								...
								{	
									errorMessage = std::string("In ParseResult::Parse(...) -Got http_parser error: ");
					...
					void Kill()
					{
						...
						if (!m_killed)
						{
							...
							#ifndef NDEBUG
							ReportInfo("TlsCapableHttpBridge<>::Kill");
							#endif // !NDEBUG
							...
							if (downstreamShutdownErr)
							{
								...
								std::string err("In TlsCapableHttpBridge<BridgeSocketType>::Kill() - When shutting down downstream socket, got error:\t");
							...
							if (downstreamCloseErr)
							{
								...
								std::string err("In TlsCapableHttpBridge<BridgeSocketType>::Kill() - When closing downstream socket, got error:\t");
							...
							if (upstreamShutdownErr)
							{
								...
								std::string dErrMessage("In TlsCapableHttpBridge<BridgeSocketType>::Kill() - When shutting down upstream socket, got error:\t");
							...
							if (upstreamCloseErr)
							{
								...
								std::string err("In TlsCapableHttpBridge<BridgeSocketType>::Kill() - When closing upstream socket, got error:\t");
					...
					void OnUpstreamHeaders(const boost::system::error_code& error, const size_t bytesTransferred)
					{
						...
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::OnUpstreamHeaders");
						#endif // !NDEBUG
						...
						if ((!error || (error == boost::asio::error::eof || (error.category() == boost::asio::error::get_ssl_category()) && (ERR_GET_REASON(error.value()) == SSL_R_SHORT_READ))))
						{
							...
							if (m_response->Parse(bytesTransferred))
							{
								if (wasSslShortRead && (!m_response->IsPayloadComplete() || !m_response->HeadersComplete()))
								{
									...
									ReportInfo("In TlsCapableHttpBridge::OnUpstreamHeaders(const boost::system::error_code&, const size_t) - Got TLS short read and payload IS NOT complete. Aborting.");
									Kill();
									return;
								}
								...
								if (wasSslShortRead)
								{
									ReportWarning("In TlsCapableHttpBridge::OnUpstreamHeaders(const boost::system::error_code&, const size_t) - Got TLS short read, but payload is complete. The server did not do a proper TLS shutdown.");
								}
								...
								{
									if (connectionHeader.first->second.compare("close") == 0)
									{
										keepAlive = false;										
									}
							...
							{
								ReportError("In TlsCapableHttpBridge::OnUpstreamHeaders(const boost::system::error_code&, const size_t) - Failed to parse response.");
							}							
						}

						if (error)
						{
							std::string errMsg("In TlsCapableHttpBridge::OnUpstreamHeaders(const boost::system::error_code&, const size_t) - Got error:\t");
					...
					{
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::OnUpstreamRead");
						#endif // !NDEBUG
						...
						if ((!error || (error == boost::asio::error::eof || (error.category() == boost::asio::error::get_ssl_category()) && (ERR_GET_REASON(error.value()) == SSL_R_SHORT_READ))))
						{
							...
							if (m_response->Parse(bytesTransferred))
							{
								if (wasSslShortRead && (!m_response->IsPayloadComplete() || !m_response->HeadersComplete()))
								{
									...
									// If wasSslShortRead == true, but the payload is deemed complete, then
									// we simply have a dumb server that didn't do a clean TLS shutdown.
									ReportInfo("In TlsCapableHttpBridge::OnUpstreamRead(const boost::system::error_code&, const size_t) - Got TLS short read and payload IS NOT complete. Aborting.");
									Kill();
									return;
								}
								...
								if (wasSslShortRead)
								{
									ReportWarning("In TlsCapableHttpBridge::OnUpstreamRead(const boost::system::error_code&, const size_t) - Got TLS short read, but payload is complete. The naughty remote server did not do a proper TLS shutdown.");
								}
								...
								if (!closeAfter && m_response->IsPayloadComplete() == false && m_response->GetConsumeAllBeforeSending() == true)
								{
									...
									{
										std::string errMsg("In TlsCapableHttpBridge::OnUpstreamRead(const boost::system::error_code&, const size_t) - Got error:\t");
							...
							{
								ReportError("In TlsCapableHttpBridge::OnUpstreamRead(const boost::system::error_code&, const size_t) - Failed to parse response.");
							}
						}
					...
					void OnUpstreamWrite(const boost::system::error_code& error)
					{
						...
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::OnUpstreamWrite");
						#endif // !NDEBUG
						...
						if (!error)
						{
							if (m_request->IsPayloadComplete() == false)
							{
								...
								{
									std::string errMsg("In TlsCapableHttpBridge::OnUpstreamWrite(const boost::system::error_code&) - Got error:\t");
									errMsg.append(e.what());
									ReportError(errMsg);
								}
							}
					...
					{
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::::OnDownstreamHeaders");
						#endif // !NDEBUG
						...
						if ((!error || (error == boost::asio::error::eof || (error.category() == boost::asio::error::get_ssl_category()) && (ERR_GET_REASON(error.value()) == SSL_R_SHORT_READ))))
						{
							...
							{			
								...
								if (wasSslShortRead && (!m_request->IsPayloadComplete() || !m_request->HeadersComplete()))
								{
									...
									// If wasSslShortRead == true, but the payload is deemed complete, then
									// we simply have a dumb server that didn't do a clean TLS shutdown.
									ReportInfo("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Got TLS short read and payload IS NOT complete. Aborting.");
									Kill();
									return;
								}
								...
								if (wasSslShortRead)
								{
									ReportWarning("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Got TLS short read, but payload is complete. The naughty client did not do a proper TLS shutdown.");
								}
								...
								std::string standardEncoding("gzip");
								...
								if (hostHeader.first != hostHeader.second)
								{
									...
									if (portInd != std::string::npos && portInd < hostWithoutPort.size())
									{
										...
										catch (...)
										{
											...
											ReportWarning("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Failed to parse port in host entry. Assuming port 80.");
										}										
									}
								...
								{
									ReportError("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Failed to read Host header from request.");
								}
							}
							else
							{
								ReportError("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Failed to parse request.");
							}
						}

						if (error)
						{
							std::string errMsg("In TlsCapableHttpBridge::OnDownstreamHeaders(const boost::system::error_code&, const size_t) - Got error:\t");
							errMsg.append(error.message());
							ReportError(errMsg);
						}
					...
					{
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::OnDownstreamRead");
						#endif // !NDEBUG
						...
						if ((!error || (error == boost::asio::error::eof || (error.category() == boost::asio::error::get_ssl_category()) && (ERR_GET_REASON(error.value()) == SSL_R_SHORT_READ))))
						{
							...
							if (m_request->Parse(bytesTransferred))
							{
								if (wasSslShortRead && (!m_request->IsPayloadComplete() || !m_request->HeadersComplete()))
								{
									...
									// If wasSslShortRead == true, but the payload is deemed complete, then
									// we simply have a dumb server that didn't do a clean TLS shutdown.
									ReportInfo("In TlsCapableHttpBridge::OnDownstreamRead(const boost::system::error_code&, const size_t) - Got TLS short read and payload IS NOT complete. Aborting.");
									Kill();
									return;
								}
								...
								if (wasSslShortRead)
								{
									ReportWarning("In TlsCapableHttpBridge::OnDownstreamRead(const boost::system::error_code&, const size_t) - Got TLS short read, but payload is complete. The naughty client did not do a proper TLS shutdown.");
								}
								...
								if (!closeAfter && m_request->IsPayloadComplete() == false && m_request->GetConsumeAllBeforeSending())
								{
									...
									{
										std::string errMsg("In TlsCapableHttpBridge::OnDownstreamRead(const boost::system::error_code&, const size_t) - Got error:\t");
										errMsg.append(e.what());
										ReportError(errMsg);
									}
								}
					...
					{
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge::OnDownstreamWrite");
						#endif // !NDEBUG
						...
						{	
							...
							if (m_response && m_response->IsPayloadComplete() == false)
							{
								...
								{
									std::string errMsg("In TlsCapableHttpBridge::OnDownstreamWrite(const boost::system::error_code&) - Got error:\t");
									errMsg.append(e.what());
									ReportError(errMsg);
								}
							}
							...
							{							
								...
								{
									#ifndef NDEBUG
									ReportInfo("In TlsCapableHttpBridge::OnDownstreamWrite(const boost::system::error_code&) - Keep-alive specified, initiating new read.");
									#endif
					...
					void OnStreamTimeout(const boost::system::error_code& error)
					{
						...
						#ifndef NDEBUG
							ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnStreamTimeout");
						#endif // !NDEBUG
						...
						if (error)
						{
							...
							else
							{
								...
								// calling ::Kill()?
								std::string errMessage("In TlsCapableHttpBridge<BridgeSocketType>::OnStreamTimeout(const boost::system::error_code&) - Got error:\t");
					...
					void OnUpstreamHandshake(const boost::system::error_code& error)
					{
						...
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake");
						#endif // !NDEBUG
						...
						if (!error && m_upstreamCert != nullptr)
						{
							...
							{
								serverCtx = nullptr;
								std::string errMessage("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake(const boost::system::error_code&) - While spoofing, got error:\t");
							...
							if (serverCtx != nullptr)
							{
								...
								{
									ReportError("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake(const boost::system::error_code&) - Failed to correctly set context.");
								}
							}
							else
							{
								ReportError("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake(const boost::system::error_code&) - Failed to fetch spoofed context.");
							}
						}
						else
						{
							if (error)
							{
								std::string errMsg("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake(const boost::system::error_code&) - Got error:\t");
								errMsg.append(error.message());
								ReportError(errMsg);
								...
								if (wasSSlError)
								{
									ReportError("Was TLS error.");
								}
							}

							if (m_upstreamCert == nullptr)
							{
								ReportError("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamHandshake(const boost::system::error_code&) - Upstream cert is nullptr!");
							}
						}
					...
					void OnDownstreamHandshake(const boost::system::error_code& error)
					{
						...
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnDownstreamHandshake");
						#endif // !NDEBUG
						...
						std::string errMessage("In TlsCapableHttpBridge<network::TlsSocket>::OnDownstreamHandshake(const boost::system::error_code&) - Got error:\t");
						errMessage.append(error.message());
						ReportError(errMessage);
					...
					void OnTlsPeek(const boost::system::error_code& error, const size_t bytesTransferred)
					{
						...
						#ifndef NDEBUG
						ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek");
						#endif // !NDEBUG
						...
						{						
							if (bytesTransferred > MinTlsHelloLength)
							{
									...
									(const std::unique_ptr< std::array<char, TlsPeekBufferSize> >& arr, const size_t position, const size_t validDataLength, int crumb = 0)->bool
								{
									...
									{
										#ifndef NDEBUG
											std::string errMessage("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - ");
											errMessage.append("Index in buffer is out of bounds at position ").append(std::to_string(position)).append(".\n\n");
											errMessage.append("Went out of bounds at check ").append(std::to_string(crumb)).append(".");
											ReportError(errMessage);
										#endif
								...
								if ((versionMajor == 3 && versionMinor > 0) || (versionMinor > 3))
								{
									if (contentType == 22 && handshakeType == 1)
									{
										...
										if (hostnameLength > 0)
										{
											...
											{
												std::string errorMessage("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - Got Error:\t");
										...
										{
											ReportError("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - Failed to extract hostname from SNI extension.");
										}
									}
									else
									{
										ReportWarning("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - Not a TLS client hello.");
									}
								}
								else
								{
									ReportWarning("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - Not a TLS client.");
								}
							}
						}
						else
						{							
							...
							{
								ReportError("In TlsCapableHttpBridge<network::TlsSocket>::OnTlsPeek(const boost::system::error_code&, const size_t) - TLS peek buffer is nullptr!");
							}
						}
					...
					{
						//ReportInfo("HandleDownstreamPassthrough");
						...
						if(!ec)
						{
							...
							{
								auto self(this->shared_from_this()/*shared_from_this()*/);
								...
								boost::asio::async_write(
									...
									m_upstreamStrand.wrap(
										[this, self, buff, closeAfter](const boost::system::error_code& err, const size_t bytesSent)
										{
											if (closeAfter)
											{
												ReportInfo("In TlsCapableHttpBridge::HandleDownstreamPassthrough(const boost::system::error_code&) - Connection closed by downstream.");
												Kill();
												return;
											}
						...
						else
						{
							if (ec)
							{
								//std::string errMsg("In TlsCapableHttpBridge::HandleDownstreamPassthrough(const boost::system::error_code&) - Got error in handler:\t");
					...
					{
						//ReportInfo("HandleUpstreamPassthrough");
								...
								boost::asio::async_write(
									...
									m_downstreamStrand.wrap(
										[this, self, buff, closeAfter](const boost::system::error_code& err, const size_t bytesSent)
										{
											if (closeAfter)
											{
												ReportInfo("In TlsCapableHttpBridge::HandleUpstreamPassthrough(const boost::system::error_code&) - Connection closed by upstream.");
												Kill();
												return;
											}
						...
						else
						{
							if (ec)
							{
								//std::string errMsg("In TlsCapableHttpBridge::HandleUpstreamPassthrough(const boost::system::error_code&) - Got error in handler:\t");
								//errMsg.append(ec.message());
								//ReportError(errMsg);
							}
						}
					...
					{	
						ReportInfo("Starting passthrough.");
						SetStreamTimeout(boost::posix_time::minutes(5));
						...
						{
							std::string errMsg("In TlsCapableHttpBridge::StartPassthroughVolley(std::shared_ptr<std::array<char, TlsPeekBufferSize>>, const size_t) - Got error while writing initial payload:\t");
							errMsg.append(e.what());
							ReportError(errMsg);
						}
						...
						{
							std::string errMsg("In TlsCapableHttpBridge::StartPassthroughVolley(std::shared_ptr<std::array<char, TlsPeekBufferSize>>, const size_t) - Got errorsss while writing initial payload:\t");
							errMsg.append(e.what());
							ReportError(errMsg);
						}
					}
					...
					void OnInitialPeek(const boost::system::error_code& error, const size_t bytesTransferred, std::shared_ptr<std::array<char, TlsPeekBufferSize>> httpPeekBuffer)
					{
						if (!error && httpPeekBuffer && httpPeekBuffer.get() && httpPeekBuffer->data() && bytesTransferred > 0)
						{
							...
							switch (parseResult)
							{
								...
								{
									ReportError("In TlsCapableHttpBridge:: InitiateHttpTransaction() - Failed when trying to peek connected client.");
									Kill();
									return;
								}
								break;
								...
								case PreviewParser::ParseResult::HttpWithUpgrade:
								{
									...
									if (needsResolveAndConnect)
									{
										...
										{
											ReportWarning("HTTP-With-Upgrade passthrough detected, but no host extracted.");
											Kill();
											return;
										}

										// Trim port, if present.
										auto portPos = parsedHost.find_last_of(":");
										...
										*/
										...
										auto self = this->shared_from_this(); //this->shared_from_this()/*shared_from_this()*/;
								...
								{	
									...
									if (std::is_same<BridgeSocketType, network::TcpSocket>::value)
									{
										...
										ReportError("In TlsCapableHttpBridge::TryInitiateHttpTransaction() - Connected client is non-tls and sending content in an unexpected protocol. Terminating because no mechanism exists to resolve the original upstream host.");
										Kill();
										return;
									}
									else
									{
										ReportInfo("Not http and is TLS.");
					...
					void SetStreamTimeout(const boost::posix_time::time_duration& expiry)
					{
						...
						m_streamTimer.async_wait(std::bind(&TlsCapableHttpBridge::OnStreamTimeout, this->shared_from_this(), std::placeholders::_1));
					}
					...
					void SetLinger(boost::asio::ip::tcp::socket& socket, const bool enabled, const int value)
					{
						...
						if (err)
						{
							std::string errorMessage("In TlsCapableHttpBridge<BridgeSocketType>::SetLinger(boost::asio::ip::tcp::socket&, const bool) - While setting linger state, got error:\t");
					...
					void SetNoDelay(boost::asio::ip::tcp::socket& socket, const bool value)
					{
						...
						if (err)
						{
							std::string errorMessage("In TlsCapableHttpBridge<BridgeSocketType>::SetNoDelay(boost::asio::ip::tcp::socket&, const bool) - While setting Nagle algorithm enabled state, got error:\t");
							errorMessage.append(err.message());
							ReportError(errorMessage);
						}
					}
