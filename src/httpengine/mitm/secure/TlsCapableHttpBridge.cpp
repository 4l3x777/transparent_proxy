...
#include "httpengine/mitm/secure/TlsCapableHttpBridge.hpp"
	...
	namespace httpengine
	{
		namespace mitm
		{
			...
			{	
                ...
                //template <typename T>
                //size_t TlsCapableHttpBridge<T>::s_streamChannelMultiplier {10};
				...
				{
					#ifndef NDEBUG
						assert(m_certStore != nullptr && "In TlsCapableHttpBridge<network::TlsSocket>::TlsCapableHttpBridge(... args) - Supplied certificate store is nullptr!");						
					#else
						if (m_certStore == nullptr)
						{
							throw std::runtime_error("In TlsCapableHttpBridge<network::TlsSocket>::TlsCapableHttpBridge(... args) - Supplied certificate store is nullptr!");
						}
					#endif
				...
				void TlsCapableHttpBridge<network::TcpSocket>::Start()
				{
					...
					{
						std::string errMessage("IN TlsCapableHttpBridge<network::TcpSocket>::Start() - Got error:\t");
				...
				{					
					...
					{
						std::string errMessage("IN TlsCapableHttpBridge<network::TlsSocket>::Start() - Got error:\t");
						errMessage.append(e.what());						
						ReportError(errMessage);
					}
				...
				const void TlsCapableHttpBridge<BridgeSocketType>::InitClientContext(TlsCapableHttpBridge<BridgeSocketType>* bridgeCtx, boost::asio::ssl::stream<boost::asio::ip::tcp::socket>& sslStream, const std::string& hostname)
				{
					...
					else
					{
						...
						newContext.reset(new boost::asio::ssl::context(sslStream.get_executor().context(), boost::asio::ssl::context::tlsv12_client));
						...
						SSL_CTX_set_cipher_list(newContext->native_handle(), "EECDH+ECDSA+AESGCM:EECDH+aRSA+AESGCM:EECDH+ECDSA+SHA384:EECDH+ECDSA+SHA256:EECDH+aRSA+SHA384:EECDH+aRSA+SHA256:!EECDH+aRSA+RC4:EECDH:EDH+aRSA:!RC4:!aNULL:!eNULL:!LOW:!3DES:!MD5:!EXP:!PSK:!SRP:!DSS:!EDH-RSA-DES-CBC3-SHA:!ECDHE-RSA-DES-CBC3-SHA:!DES-CBC3-SHA:!ECDHE-RSA-RC4-SHA:!RC4-MD5:!RC4-SHA"/*"HIGH:!aNULL:!kRSA:!PSK:!SRP:!MD5:!RC4"*/);
						...
						if (defaultContext != nullptr)
						{
							...
							{
								bridgeCtx->ReportError("In TlsCapableHttpBridge<BridgeSocketType>::InitClientContext(boost::asio::ssl::stream<boost::asio::ip::tcp::socket>&, const std::string&) - Failed to get default certificate store.");
							}
						}
						else
						{
							bridgeCtx->ReportError("In TlsCapableHttpBridge<BridgeSocketType>::InitClientContext(boost::asio::ssl::stream<boost::asio::ip::tcp::socket>&, const std::string&) - Failed to get default client context.");
						}
				...
				void TlsCapableHttpBridge<network::TcpSocket>::OnUpstreamConnect(const boost::system::error_code& error)
				{
					...
					#ifndef NDEBUG
					ReportInfo("TlsCapableHttpBridge<network::TcpSocket>::OnUpstreamConnect");
					#endif // !NDEBUG
					...
					{					
						if (m_request->IsPayloadComplete() == false && m_request->GetConsumeAllBeforeSending() == true)
						{
							...
							{
								std::string errMsg("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamConnect(const boost::system::error_code&) - Got error:\t");
				...
				void TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamConnect(const boost::system::error_code& error)
				{
					...
					#ifndef NDEBUG
					ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamConnect");
					#endif // !NDEBUG
					...
					{						
						...
						{
							std::string errMsg("In TlsCapableHttpBridge<network::TlsSocket>::OnUpstreamConnect(const boost::system::error_code&) - While setting verification callback, got error:\t");
							errMsg.append(error.message());
							ReportError(errMsg);
						}
					}
				...
				void TlsCapableHttpBridge<network::TcpSocket>::OnResolve(const boost::system::error_code& error, boost::asio::ip::tcp::resolver::iterator endpointIterator)
				{
					...
					#ifndef NDEBUG
					ReportInfo("TlsCapableHttpBridge<network::TcpSocket>::OnResolve");
					#endif // !NDEBUG
					...
					{
						std::string errMsg("In TlsCapableHttpBridge<network::TcpSocket>::OnResolve(const boost::system::error_code&, boost::asio::ip::tcp::resolver::iterator) - Got error:\t");
						errMsg.append(error.message());
						ReportError(errMsg);
					}
				...
				{	
					#ifndef NDEBUG
					ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::OnResolve");
					#endif // !NDEBUG
					...
					{
						std::string errMsg("In TlsCapableHttpBridge<network::TlsSocket>::OnResolve(const boost::system::error_code&, boost::asio::ip::tcp::resolver::iterator) - Got error:\t");
						errMsg.append(error.message());
						ReportError(errMsg);
					}
				...
				{	
					...
					#ifndef NDEBUG
					ReportInfo("TlsCapableHttpBridge<network::TlsSocket>::VerifyServerCertificateCallback");
					#endif // !NDEBUG
