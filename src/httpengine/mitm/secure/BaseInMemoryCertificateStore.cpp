...
#include <httpengine/mitm/secure/BaseInMemoryCertificateStore.hpp>
...
namespace te
{
	namespace httpengine
	{
		namespace mitm
		{
			namespace secure
			{
				const std::string BaseInMemoryCertificateStore::ContextCipherList{ "EECDH+ECDSA+AESGCM:EECDH+aRSA+AESGCM:EECDH+ECDSA+SHA384:EECDH+ECDSA+SHA256:EECDH+aRSA+SHA384:EECDH+aRSA+SHA256:!EECDH+aRSA+RC4:EECDH:EDH+aRSA:!RC4:!aNULL:!eNULL:!LOW:!3DES:!MD5:!EXP:!PSK:!SRP:!DSS:!EDH-RSA-DES-CBC3-SHA:!ECDHE-RSA-DES-CBC3-SHA:!DES-CBC3-SHA:!ECDHE-RSA-RC4-SHA:!RC4-MD5:!RC4-SHA"/*"HIGH:!SSLv2:!SSLv3:!aNULL:!kRSA:!PSK:!SRP:!MD5:!RC4"*/ }; // "HIGH:!aNULL:!MD5:!RC4" // "HIGH:!SSLv2!SRP:!PSK"
				

				BaseInMemoryCertificateStore::BaseInMemoryCertificateStore() :
					BaseInMemoryCertificateStore("RU", "AO Kaspersky Lab1", "Kaspersky Anti-Virus Personal Root Certificate1")
				{
				...
				boost::asio::ssl::context* BaseInMemoryCertificateStore::GetServerContext(const std::string& hostname, X509* originalCertificate)
				{
					...
					if (m_thisCa != nullptr && m_thisCaKeyPair != nullptr && originalCertificate != nullptr)
					{
						...
						{
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to load remote certificate X509_NAME data.");
						}
						...
						{
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to generate EC key for spoofed certificate.");
						}
						...
						if (spoofedCert == nullptr)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to generate X509 structure.");
						}
						...
						if (sanDnsString.size() > 0)
						{
							if (!Addx509Extension(spoofedCert, NID_subject_alt_name, sanDnsString))
							{
								...
								throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to set SAN's for spoofed certificate.");
							}
						}
						...
						if (m_thisCaKeyPair == nullptr || X509_sign(spoofedCert, m_thisCaKeyPair, EVP_sha256()) == 0)
						{
							EVP_PKEY_free(spoofedCertKeypair);
							X509_free(spoofedCert);
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to sign certificate.");
						}
						...
						if (ctx == nullptr)
						{
							EVP_PKEY_free(spoofedCertKeypair);
							X509_free(spoofedCert);
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to allocate new server context for spoofed certificate.");
						}
						...
						if (SSL_CTX_set_cipher_list(ctx->native_handle(), ContextCipherList.c_str()) != 1)
						{
							EVP_PKEY_free(spoofedCertKeypair);
							X509_free(spoofedCert);
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to set context cipher list.");
						}
						*/
						...
						if (SSL_CTX_use_certificate(ctx->native_handle(), spoofedCert) != 1)
						{
							EVP_PKEY_free(spoofedCertKeypair);
							X509_free(spoofedCert);
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to set server context certificate.");
						}
						...
						if (SSL_CTX_use_PrivateKey(ctx->native_handle(), spoofedCertKeypair) != 1)
						{
							EVP_PKEY_free(spoofedCertKeypair);
							X509_free(spoofedCert);
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Failed to set server context private key.");
						}
						...
						if (!atLeastOneInsert)
						{
							...
							X509_free(spoofedCert);
							delete ctx;
							throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Context already exists for specified host.");
						}
					...
					{
						throw std::runtime_error("In BaseInMemoryCertificateStore::GetServerContext(std::string, X509*) - Cannot spoof certificate. Either member CA , member CA keypair or certificate to spoof is nullptr.");
					}
				}
				...
				EVP_PKEY* BaseInMemoryCertificateStore::GenerateEcKey(const int namedCurveId) const
				{
					...
					{
						throw std::runtime_error("In BaseInMemoryCertificateStore::GenerateEcKey(const int) - Failed to allocate EC_KEY structure.");
					}
					...
					if (pkey == nullptr)
					{
						throw std::runtime_error("In BaseInMemoryCertificateStore::GenerateEcKey(const int) - Failed to allocate EVP_PKEY structure.");
					}
					else
					{
						if (EVP_PKEY_set1_EC_KEY(pkey, eckey) != 1)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::GenerateEcKey(const int) - Failed to assign EC_KEY to EVP_PKEY structure.");
						}
					}
					...
					) const
				{
					...
					{
						throw std::runtime_error("In BaseInMemoryCertificateStore::GenerateSelfSignedCert(EVP_PKEY*, std::string, std::string, std::string) - Failed to allocate X509 structure.");
					}
					...
					) const
				{
					...
					if (!x509)
					{
						throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to allocate X509 structure.");
					}
					...
					if (X509_set_version(x509, 2) != 1)
					{
						...
						throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA version number.");
					}
					...
					if (ASN1_INTEGER_set(X509_get_serialNumber(x509), sn) != 1)
					{
						...
						throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA certificate serial number.");
					}
					...
					if (X509_set_pubkey(x509, certificateKeypair) != 1)
					{
						...
						throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA public key.");
					}
					...
					if (name != nullptr)
					{
						if (X509_NAME_add_entry_by_txt(name, "C", MBSTRING_ASC, reinterpret_cast<const unsigned char*>(countryCode.c_str()), -1, -1, 0) == 0)
						{
							//X509_free(x509);

							//throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA certificate country code.");
						}

						if (X509_NAME_add_entry_by_txt(name, "O", MBSTRING_ASC, reinterpret_cast<const unsigned char*>(organizationName.c_str()), -1, -1, 0) == 0)
						{
							//X509_free(x509);

							//throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA certificate organization.");
						}
						...
						if (X509_NAME_add_entry_by_txt(name, "CN", MBSTRING_ASC, reinterpret_cast<const unsigned char*>(commonName.c_str()), -1, -1, 0) == 0)
						{
							...
							//throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA common name.");
						}
					}
					else
					{
					...
					if (isCA)
					{
						...
						if (!Addx509Extension(x509, NID_basic_constraints, "critical,CA:TRUE"))
						{
							X509_free(x509);

							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA constraints.");
						}
						...
						if (X509_set_issuer_name(x509, name) != 1)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA isser name information.");
						}

						if (!Addx509Extension(x509, NID_key_usage, "critical,keyCertSign,cRLSign"))
						{
							X509_free(x509);

							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA key usage.");
						}
					}
					else
					{
						if (m_thisCa == nullptr)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Cannot issue certificate, as the member CA is nullptr.");
						}
						...
						if (issuerName == nullptr)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to get X509_NAME structure from member CA.");
						}
						...
						if (X509_set_issuer_name(x509, issuerName) != 1)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set issuer name for non CA certificate.");
						}
					}
					...
					if (!Addx509Extension(x509, NID_subject_key_identifier, "hash"))
					{
						X509_free(x509);

						throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string)  - Failed to set self signed CA subject key identifier.");
					}
					...
					if (issuerKeypair != nullptr)
					{
						if (X509_sign(x509, issuerKeypair, EVP_sha256()) == 0)
						{
							...
							throw std::runtime_error("In BaseInMemoryCertificateStore::IssueCertificate(EVP_PKEY*, EVP_PKEY*, const bool, std::string, std::string, std::string) - Failed to sign certificate.");
						}
					}
